...
export default function AdminInvoiceView({ id }) {
  ...
  async function load() {
    try {
      setLoading(true);
      const r = await AdminAPI.invoices();
      const found = (r.items || []).find(i => String(i.id) === String(id));
      setInv(found || null);
  ...
  if (loading || !inv) return <div className="flex justify-center py-12"><div className="animate-spin h-8 w-8 rounded-full border-b-2 border-blue-600"></div></div>;
  const currency='\u20B9';
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          <Link href="/admin/finance" className="text-gray-500 hover:text-gray-700">← Back</Link>
      ...
      <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
          ...
          <div className="text-sm text-gray-600">
            <div><span className="text-gray-500">Date:</span> {new Date(inv.createdAt).toLocaleDateString()}</div>
