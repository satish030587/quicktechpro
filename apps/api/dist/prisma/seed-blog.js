...
async function resolveAdminUserId() {
    ...
    const passwordHash = await bcrypt_1.default.hash('Admin!234', 10);
...
async function createBlogPost(adminUserId, data) {
    const { title, slug, excerpt, content, coverImage, categoryId, tags, featured = false, allowComments = true, status = client_1.BlogStatus.PUBLISHED, publishedAt: publishedAtInput } = data;
    ...
    const publishedAt = (() => {
        ...
        if (status === client_1.BlogStatus.SCHEDULED) {
